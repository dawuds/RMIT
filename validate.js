#!/usr/bin/env node
/**
 * validate.js — Data integrity checks for RMIT compliance database
 *
 * Checks:
 ...
 *   4. All clause keys in evidence/index.json and requirements/index.json exist in clauses/index.json
...
const clauses = JSON.parse(fs.readFileSync('./clauses/index.json', 'utf8'));
...
for (const clauseId of Object.keys(evidence)) {
  const entry = evidence[clauseId];
...
if (requirementKeyErrors === 0) {
  ok(`All ${Object.keys(requirements).length} requirement keys match clause IDs`);
}
